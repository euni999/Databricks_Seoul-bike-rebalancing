...
Raw CSV (서울 열린데이터광장)
        ↓
Bronze  원본 그대로 Delta 적재 (EUC-KR 처리)
        ↓
Silver  타입캐스팅 + 좌표 조인 + 파생 컬럼
        ↓
Gold    net_flow 집계 / 재배치 우선순위 / ML 예측 결과
        ↓
ML      XGBoost 수요 예측 + MLflow 실험 추적
        ↓
Workflows  6-Task DAG 자동화 (Cron: 0 1 1 * *)
```

Databricks Workflows DAG:
```
00_collect_data → 01_bronze_ingest → 02_silver_transform
→ 03_gold_aggregate → 04_demand_forecast → 05_cleanup
```
